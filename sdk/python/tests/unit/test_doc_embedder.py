...
class TestTextChunker:
    ...
    def test_empty_text_returns_no_chunks(self):
        ...
        chunks = chunker.load_parse_and_chunk(source="", source_id="doc1")
...
class TestMultiModalEmbedder:
    ...
    def test_custom_modality_registration(self):
        """Registering a custom modality handler routes embed calls to it."""
        embedder = MultiModalEmbedder()

        mock_handler = Mock(return_value=np.array([[1.0, 2.0]]))
        embedder.register_modality("audio", mock_handler)

        assert "audio" in embedder.supported_modalities()

        result = embedder.embed(["audio_data"], "audio")
...
class TestDocEmbedder:
    ...
    def test_embed_documents_full_chain(
        ...
        mock_logical_fn.return_value = transformed_df
        ...
        mock_logical_fn.assert_called_once()
        pd.testing.assert_frame_equal(mock_logical_fn.call_args[0][0], embedded_df)
    ...
    def test_embed_documents_custom_column_mapping(
        ...
        mock_logical_fn = MagicMock()
        mock_logical_fn.return_value = pd.DataFrame(
            {"passage_id": ["c1"], "embedding": [[0.1]]}
        )
        ...
        custom_mapping = {"content": ("text", "content_embedding")}
        doc_embedder.embed_documents(
        ...
        embed_call = mock_embedder.embed_dataframe.call_args
        assert embed_call[1]["column_mapping"] == custom_mapping
